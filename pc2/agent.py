"""
Funciones principales para ejecutar el agente multimedia de manera abstraída.
"""
...
def ejecutar_agente_multimedia(grafo, archivo_path, pregunta, api_key=None, debug=False):
    """
    Ejecuta el agente multimedia con máxima abstracción.
    
    Soporta tanto imágenes como videos. Para videos, incluye análisis de riesgos.
    
    Args:
        grafo: El grafo sin compilar construido por el usuario
        archivo_path: Ruta al archivo (imagen o video)
        pregunta: Pregunta del usuario
        api_key: API key opcional (si no está en variables de entorno)
        debug: Si True, muestra logs detallados de la ejecución
        
    Returns:
        str: Respuesta del agente (incluye análisis de riesgos para videos)
    """
